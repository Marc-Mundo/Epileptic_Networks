...
from geom import Cell, Synapse, SynapseNMDA, Ow
...
def vary_na_conductances_olm():
    """
    This function creates a Ow cell and varies the sodium conductance in the soma.
    ...
    # Create a new Ow cell
    olm_cell = Ow(x=0, y=0, z=0, id=0)
    ...
    for p in np.arange(0.1, 10, 0.5):
        ...
        # Adjust the sodium conductance in the specific compartment (e.g., soma)
        olm_cell.soma(0.5).Nafbwb.gna *= p  # Adjust based on your specific mechanism
        ...
        voltage_vec.record(olm_cell.soma(0.5)._ref_v)  # Records the membrane potential
        ...
        for synapse_name in synapses_dict:
            ...
            synapse = getattr(olm_cell, synapse_name)
        ...
        for vec_stim, netcon in zip(vec_stim_dict.values(), netcon_dict.values()):
            # Loop over the values of the dictionaries
            del vec_stim
            del netcon

        # Clear Ow cell instance
        del olm_cell
    ...
    plt.title("OLM Cell Activity")
    plt.legend()
    plt.show()


# Call the function
vary_na_conductances_olm()
