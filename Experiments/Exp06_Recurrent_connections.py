# Design new experiment where we change the recurrent GABAergic connections of many to one basket cells (at certain external noise, and conductance)
# and see how the network behaves
...
def init_network(**kwargs):
    ...
    bwb_to_bwb_weight = kwargs.get("bwb_to_bwb_weight", 1.0)
    ...
    netParams.connParams["Bwb->Bwb GABA"] = (
        {  # Change these over a range of values at the scale
            "method": "many_to_one",
            "count": 60,
            "weight": bwb_to_bwb_weight * 3 * 1.5 * 1.0e-3,
            "synapse": "somaGABAf",
            "threshold": 0,
            "delay": 2,
        }
...
def run_many_smarter():
    base_data_path = "/mnt/internserver1_1tb/Data/MarcData/Data06_Recurrent_connections"  # If running from internserver2, will write to internserver1 ssd

    ensure_directory_exists(base_data_path)  # Ensure the base directory exists

    Bwb_Bwb_weights = np.array(
        [1.00, 1.05, 1.10, 1.15, 1.20]
    )  # Basket cell recurrent connection strength

    all_nps_seed_trials = []
    for gk in np.arange(0.75, 1.50, 0.25):  # Select quadrant values +- 25%
        for gna in np.arange(0.75, 1.50, 0.25):  # Select quadrant values +- 25%
            for bwb_bwb_weight in Bwb_Bwb_weights:
                olm_pyr_weight = 0.1  # fig. 7 Sanjay, reduced olm to pyr connections, 15x external input
                pyr_noise_scale = 20  # Fixed at 20x as the baseline, where at unchanged gna and gk, depolarization blocks occurs  near 100% of the time

                variant = f"gna_{gna:.2f}_gk_{gk:.2f}_bwb_bwb_weight_{bwb_bwb_weight:.2f}"  # add in Bwb-Bwb connections
                ...
                nps["bwb_bwb_weight"] = bwb_bwb_weight
    ...
    # Run the sim in parallel on internserver2, save to internserver1
